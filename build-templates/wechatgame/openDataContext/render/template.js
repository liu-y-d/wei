...
export function setRankTemplate(it) {
    ...
    if (arr1) {
        ...
        while (index < l1) {
            ...
            out += ' <text class="listItemNum" value="' + (index + 1) + '"></text> <image class="listHeadImg" src="' + (item.avatarUrl) + '"></image> <text class="listItemName" value="' + (item.nickname) + '"></text> <text class="listItemScore" value="' + (JSON.parse(item.KVDataList[0].value).actual_score) + '"></text> <text class="listScoreUnit" value="关"></text> </view> ';
        }
    }
    out += ' </scrollview> <text class="listTips" value="仅展示前50位好友排名"></text> <view class="listItem selfListItem"> <text  class="listItemNum" value="' + (it.selfIndex + 1) + '"></text> <image class="listHeadImg" src="' + (it.self.avatarUrl) + '"></image> <text class="listItemName" value="' + (it.self.nickname) + '"></text> <text class="listItemScore" value="' + (JSON.parse(it.self.KVDataList[0].value).actual_score) + '"></text> <text class="listScoreUnit" value="关"></text> </view> </view></view>';
